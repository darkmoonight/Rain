...
void main() {
  ...
  group('AqiHelper', () {
    test('resolveAqi picks european or american value', () {
      expect(AqiHelper.resolveAqi(AqiHelper.european, 28.0, 45.0), 28.0);
      expect(AqiHelper.resolveAqi(AqiHelper.american, 28.0, 45.0), 45.0);
    });

    test('severityColor returns expected EU colors', () {
      expect(
        AqiHelper.severityColor(AqiHelper.european, 10),
        const Color(0xFF5DB88A),
      );
      expect(
        AqiHelper.severityColor(AqiHelper.european, 50),
        const Color(0xFFD4B84A),
      );
      expect(
        AqiHelper.severityColor(AqiHelper.european, 120),
    ...
    test('severityColor returns expected US colors', () {
      expect(
        AqiHelper.severityColor(AqiHelper.american, 30),
        const Color(0xFF5CB85C),
      );
      expect(
        AqiHelper.severityColor(AqiHelper.american, 80),
        const Color(0xFFC9B84A),
      );
      expect(
        AqiHelper.severityColor(AqiHelper.american, 350),
    ...
    test('recommendation returns advice for severity band', () {
      expect(AqiHelper.recommendation(AqiHelper.european, 16), isNotEmpty);
    ...
    test('pollutantBarFraction and pollutantColor use EEA breakpoints', () {
      ...
      expect(AqiHelper.pollutantColor('pm25', 26), const Color(0xFFD9955C));
      expect(AqiHelper.pollutantColor('so2', 60), const Color(0xFF5DB88A));
    });

    test('pollutantValue reads card fields', () {
      final card = WeatherCard(pm25: [8.4], no2: [22.0]);
